...
import yaml
from flet import *
from flet import dropdown, icons, colors
...
def main(page: Page):
    ...
    page.appbar = AppBar(
        ...
        actions=[
            ...
            IconButton(
                icons.COLOR_LENS,
                tooltip="change the theme color",
                on_click=lambda e: open_theme_color_dialog(e),
            ),
            IconButton(
                icons.FILE_OPEN,
    ...
    def pick_files_handle(e: FilePickerResultEvent):
        ...
        if e.files:
            ...
            with open(filePath.path, "r", encoding="utf-8") as f:
                data = f.read()
                # shirnk the input data, replace the tab to one space
                data = data.replace("\t", " ")
    ...
    def open_theme_color_dialog(e):
        theme_color_dialog.open = True
        page.update()

    def theme_color_changed(e):
        theme_color = e.control.value
        page.theme.color_scheme_seed = theme_color
        theme_color_dialog.open = False
        page.update()
    ...
    theme_colors = [colors.PURPLE, colors.BLUE, colors.RED, colors.GREEN, colors.YELLOW, colors.PINK, colors.BLACK, colors.CYAN, colors.LIME]
    theme_color_dialog = AlertDialog(
        title=Text("Choose the color of theme"),
        content=RadioGroup(
            value=theme_color,
            content=Column([Card(Container(content=Radio(value=color, label=color), bgcolor=color), elevation=5) for color in theme_colors]),
            on_change=theme_color_changed,
        ),
    )
    page.overlay.append(pick_files_dialog)
    page.overlay.append(save_files_dialog)
    page.overlay.append(about_dialog)
    page.overlay.append(theme_color_dialog)
